...
import { $, defer, registerCancellable } from './util.js';
...
let _sceneJumpHandler = null; // (delta) => void, registered by player

// Player registers here so input can fire ArrowLeft/Right scene-jumps.
export function setSceneJumpHandler(fn) { _sceneJumpHandler = fn; }
...
export function initInput() {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    // 左右键：跳到上一幕 / 下一幕。无视 ring 状态——剧情结构性导航优先。
    if (e.code === 'ArrowLeft' || e.code === 'ArrowRight') {
      e.preventDefault();
      if (_sceneJumpHandler) _sceneJumpHandler(e.code === 'ArrowLeft' ? -1 : +1);
      return;
    }
...
export function waitForSwipe() {
  ...
  // Scene-jump cancel: reject the swipe waiter so the player can break out.
  ...
  return _swipeWaiter.promise.finally(() => {
    offCancel();
    hideSwipeTip();
    hideSwipeHint();
  }).then(() => {
    // Fire-and-forget the water ripple — runs only on success (not on jump).
    playWaterRipple();
  });
}
...
export function waitForBoot() {
  ...
  return new Promise((resolve) => {
    const onTap = () => {
      ...
      // 首次手势内请求全屏；iOS Safari 无该 API → 静默失败。退出后不再尝试。
      const root = document.documentElement;
      const req = root.requestFullscreen || root.webkitRequestFullscreen;
      console.log('[fullscreen] req exists?', !!req, 'fullscreenEnabled?', document.fullscreenEnabled);
      if (req) {
        try {
          Promise.resolve(req.call(root))
            .then(() => console.log('[fullscreen] entered'))
            .catch((e) => console.warn('[fullscreen] rejected', e && e.message));
        } catch (e) { console.warn('[fullscreen] threw', e && e.message); }
      }
