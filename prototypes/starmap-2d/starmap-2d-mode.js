// Mode state machine for 2D star map: content ↔ starmap (sphere / overview / blackhole).

import { initStarMap, startRendering, stopRendering, hitTestNebula, animateNebulaEnter, animateNebulaExit, getWorldGroup } from './starmap-2d.js';
import { initControls, setControlsEnabled, applyZoom, onPinchStart, onPinchEnd, onLevelChange, getLevel, toggleOverview, enterBlackhole, exitBlackhole } from './starmap-2d-controls.js';
import { initPinch, setPinchEnabled } from './pinch.js';
import { initPuzzle, enterPuzzleMode, exitPuzzleMode, revealAnswers } from './starmap-2d-puzzle.js';
...
export function init(stage, canvas) {
  ...
  const { camera } = initStarMap(canvas);
  const worldGroup = getWorldGroup();
  initControls(canvas, camera, worldGroup);
...
function goToBlackhole() {
  ...
  enterBlackhole(NEBULA.x, NEBULA.y);
  animateNebulaEnter(() => {
    if (_arcEl) {
